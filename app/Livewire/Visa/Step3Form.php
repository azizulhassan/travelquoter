...
class Step3Form extends Component implements HasForms
{
    ...
    public function save()
    {
        ...
        if (session()->has('step-1-visa') && session()->has('step-2-visa')) {
            $visa = Visa::create(session()->get('step-1-visa'));
            ...
            foreach (session()->get('step-2-visa') as $agent_id) {
                QuoteAgent::create([
                    'agent_id' => $agent_id,
                    'quote_id' => $quote->id
                ]);
            ...
            session()->forget(['step-1-visa', 'step-2-visa', 'no_of_travelers']);

            Notification::make()->title('Quote sent sucessfully. We will be contacting you soon.')->success()->send();

            return $this->redirect(route('visa.step-1'), false);
        } else {
    ...
    {
        return view('livewire.visa.step3-form');
    }
}
