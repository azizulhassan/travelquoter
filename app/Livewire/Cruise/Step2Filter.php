...
class Step2Filter extends Component implements HasForms
{
    ...
    public function mount()
    {
        ...
        if (session()->has('step-2-cruise')) {
            $this->form->fill([
                'location' => 'All',
                'category' => 'All',
                'agents' => session()->get('step-2-cruise')
            ]);
        } else {
            $this->form->fill([
                'location' => 'All',
    ...
    public function submit()
    {
        ...
        session(['step-2-cruise' => $this->agents]);

        Notification::make()->title('Submit your details.')->info()->send();

        return $this->redirect(route('cruise.step-3'), false);
    }

    public function render()
    {
        return view('livewire.cruise.step2-filter');
    }
}
