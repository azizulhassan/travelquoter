...
class Step3Form extends Component implements HasForms
{
    ...
    public function save()
    {
        ...
        if (session()->has('step-1-cruise') && session()->has('step-2-cruise')) {
            $cruise = Cruise::create(session()->get('step-1-cruise'));
            ...
            foreach (session()->get('step-2-cruise') as $agent_id) {
                QuoteAgent::create([
                    'agent_id' => $agent_id,
                    'quote_id' => $quote->id
                ]);
            ...
            session()->forget(['step-1-cruise', 'step-2-cruise', 'no_of_travelers']);

            Notification::make()->title('Quote sent sucessfully. We will be contacting you soon.')->success()->send();

            return $this->redirect(route('cruise.step-1'), false);
        } else {
    ...
    {
        return view('livewire.cruise.step3-form');
    }
}
