...
class Step2Filter extends Component implements HasForms
{
    ...
    public function mount()
    {
        ...
        if (session()->has('step-2-flight')) {
            $this->form->fill([
                'location' => 'All',
                'category' => 'All',
                'agents' => session()->get('step-2-flight')
            ]);
        } else {
            $this->form->fill([
                'location' => 'All',
    ...
    public function submit()
    {
        ...
        session(['step-2-flight' => $this->agents]);

        Notification::make()->title('Submit your details.')->info()->send();

        return $this->redirect(route('flight.step-3'), false);
    }
    public function render()
    {
        return view('livewire.flight.step2-filter');
    }
}
