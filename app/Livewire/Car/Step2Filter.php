...
class Step2Filter extends Component implements HasForms
{
    ...
    public function mount()
    {
        ...
        if (session()->has('step-2-car')) {
            $this->form->fill([
                'location' => 'All',
                'category' => 'All',
                'agents' => session()->get('step-2-car')
            ]);
        } else {
            $this->form->fill([
                'location' => 'All',
    ...
    public function submit()
    {
        ...
        session(['step-2-car' => $this->agents]);

        Notification::make()->title('Submit your details.')->info()->send();

        return $this->redirect(route('car.step-3'), false);
    }

    public function render()
    {
        return view('livewire.car.step2-filter');
    }
}
