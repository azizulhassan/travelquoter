...
class Step3Form extends Component implements HasForms
{
    ...
    public function save()
    {
        ...
        if (session()->has('step-1-insurance') && session()->has('step-2-insurance')) {
            $insurance = Insurance::create(session()->get('step-1-insurance'));
            ...
            foreach (session()->get('step-2-insurance') as $agent_id) {
                QuoteAgent::create([
                    'agent_id' => $agent_id,
                    'quote_id' => $quote->id
                ]);
            ...
            session()->forget(['step-1-insurance', 'step-2-insurance']);

            Notification::make()->title('Quote sent sucessfully. We will be contacting you soon.')->success()->send();

            return $this->redirect(route('insurance.step-1'), false);
        } else {
    ...
    {
        return view('livewire.insurance.step3-form');
    }
}
