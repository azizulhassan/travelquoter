...
class Step2Filter extends Component implements HasForms
{
    ...
    public function mount()
    {
        ...
        if (session()->has('step-2-insurance')) {
            $this->form->fill([
                'location' => 'All',
                'category' => 'All',
                'agents' => session()->get('step-2-insurance')
            ]);
        } else {
            $this->form->fill([
                'location' => 'All',
    ...
    public function submit()
    {
        ...
        session(['step-2-insurance' => $this->agents]);

        Notification::make()->title('Submit your details.')->info()->send();

        return $this->redirect(route('insurance.step-3'), false);
    }

    public function render()
    {
        return view('livewire.insurance.step2-filter');
    }
}
