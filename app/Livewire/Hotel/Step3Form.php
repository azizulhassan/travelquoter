...
class Step3Form extends Component implements HasForms
{
    ...
    public function save()
    {
        ...
        if (session()->has('step-1-hotel') && session()->has('step-2-hotel')) {
            $hotel = Hotel::create(session()->get('step-1-hotel'));
            ...
            foreach (session()->get('step-2-hotel') as $agent_id) {
                QuoteAgent::create([
                    'agent_id' => $agent_id,
                    'quote_id' => $quote->id
                ]);
            ...
            session()->forget(['step-1-hotel', 'step-2-hotel', 'no_of_travelers', 'accommodation_types']);

            Notification::make()->title('Quote sent sucessfully. We will be contacting you soon.')->success()->send();

            return $this->redirect(route('hotel.step-1'), false);
        } else {
    ...
    {
        return view('livewire.hotel.step3-form');
    }
}
